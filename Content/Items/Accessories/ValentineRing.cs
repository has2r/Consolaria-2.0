        ...
        public override bool PreDrawTooltipLine(DrawableTooltipLine line, ref int yOffset) {
            if (line.Name.Equals(SPECIALKEY2)) {
                ...
                for (float num5 = 0f; num5 < 1f; num5 += 0.25f) {
                    ...
                    batch.Draw(icon, getPosition(0f) + vector2, null, color.MultiplyAlpha(0f) * 0.125f, Helper.Wave(-maxRotation, maxRotation, rotationWaveFrequency, 0f), icon.Size() / 2, scale, default, 0);
                }
                ...
                for (float num5 = 0f; num5 < 1f; num5 += 0.25f) {
                    Vector2 vector2 = (num5 * ((float)Math.PI * 2f)).ToRotationVector2() * 2f * scale;
                    batch.Draw(icon, getPosition(2f) + offset + vector2, null, color.MultiplyAlpha(0f) * 0.125f, Helper.Wave(-maxRotation, maxRotation, rotationWaveFrequency, 2f), icon.Size() / 2, scale, default, 0);
                }
