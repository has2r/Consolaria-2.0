        ...
        private void DoRocketJump () {
            if (!inAir) {
                ...
                for (int i = 0; i < 30; i++) {
                    ...
                    dust = Dust.NewDust(dustPos, Player.width, 4, DustID.Shadowflame, 0f, -1.5f, 50, default(Color), 1.5f);
                    Main.dust [dust].velocity *= 2.5f;
                    if (Main.dust [dust].position != Player.Center)
                        Main.dust [dust].velocity = Player.DirectionTo(Main.dust [dust].position) * 5f;
                }
                for (int g = 0; g < 6; g++) {
                    int goreIndex = Gore.NewGore(Player.GetSource_Misc("Rocket Jump"), dustPos, default(Vector2), Main.rand.Next(61, 64), 1f);
