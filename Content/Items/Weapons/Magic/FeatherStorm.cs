    ...
    public class FeatherStorm : ModItem {
        ...
        public override bool Shoot (Player player, EntitySource_ItemUse_WithAmmo source, Vector2 position, Vector2 velocity, int type, int damage, float knockback) {
            ...
            for (int index = 0; index < numberProjectiles; ++index) {
                ...
                vector2_1.X = (float) ((vector2_1.X + player.Center.X) / 2.0) + (float) Main.rand.Next(-100, 101);
                ...
                float SpeedX = num16 + (float) Main.rand.Next(-20, 21) * 0.05f;
                float SpeedY = num17 + (float) Main.rand.Next(-10, 21) * 0.05f;
