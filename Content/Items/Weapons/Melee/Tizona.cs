	...
	public class Tizona : ModItem {
		...
		public override bool Shoot (Player player, EntitySource_ItemUse_WithAmmo source, Vector2 position, Vector2 velocity, int type, int damage, float knockback) {
			...
			for (int i = 0; i < projectilesCount; i++) {
				...
				Projectile.NewProjectile(player.GetSource_ItemUse(Item), position, perturbedSpeed, ModContent.ProjectileType<Projectiles.Friendly.TizonaShoot>(), (int) (damage / projectilesCount), knockback / 2, player.whoAmI, player.direction * player.gravDir, player.itemAnimationMax * 2f);
			}
			return false;
		}
	}
