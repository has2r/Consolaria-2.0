...
namespace Consolaria.Content.NPCs.Ocram
{
	...
	public class Ocram : ModNPC
	{
		...
		public override void AI() {
			...
			if (NPC.target < 0 || NPC.target == 255 || player.dead || !player.active) NPC.TargetClosest(true);
			...
			bool dead2 = player.dead;
			float num317 = NPC.position.X + (float)(NPC.width / 2) - player.position.X - (float)(player.width / 2);
			float num318 = NPC.position.Y + (float)NPC.height - 59f - player.position.Y - (float)(player.height / 2);
			float num319 = (float)Math.Atan2((double)num318, (double)num317) + 1.57f;

			if (num319 < 0f) num319 += 6.2f;			
			else {
				if (num319 > 6.2f) 
					num319 -= 6.2f;
			}
			...
			else
			{
				if (NPC.ai[0] == 0f)
				{
					if (NPC.ai[1] == 0f)
					{
						...
						float num325 = player.position.X + (player.width / 2) - 300f - (num324 * 300) - vector32.X;
						float num326 = player.position.Y + (player.height / 2) - 300f - vector32.Y;
						...
						else
						{
							if (NPC.ai[2] < 500)
							{
								...
								if (NPC.ai[3] >= 120 && NPC.ai[3] <= 140)
								{
									...
									Projectile.NewProjectile(NPC.GetSpawnSource_ForProjectile(), vector8.X, vector8.Y, (float)((Math.Cos(rotation) * Speed) * -1), (float)((Math.Sin(rotation) * Speed) * -1), 100, damage, 0f, 0);						
									if (NPC.ai[3] >= 140)
									{
										NPC.ai[3] = 0;
									}
							...
							if (NPC.ai[2] > 500)
							{
								...
								if (NPC.ai[2] > 560 && NPC.ai[2] <= 620)
								{
									...
									if (NPC.ai[3] > 4)
									{
										...
										Projectile.NewProjectile(NPC.GetSpawnSource_ForProjectile(), NPC.Center.X, NPC.Center.Y, velocity.X - Main.rand.Next(-2, 2), velocity.Y - Main.rand.Next(-2, 2), 100, (int)(NPC.damage / 2), 0f, Main.myPlayer, 0f, NPC.whoAmI);
									}
								}
					...
					else
					{
						if (NPC.ai[1] == 1f)
						{
							NPC.rotation = num319;
							float num332 = 14f;
							SoundEngine.PlaySound(SoundLoader.GetLegacySoundSlot(Mod, "Assets/Sounds/OcramScythe"), NPC.position);
							Projectile.NewProjectile(NPC.GetSpawnSource_ForProjectile(), NPC.Center.X, NPC.Center.Y, -12, 0, 44, 20, 4, Main.myPlayer);
							Projectile.NewProjectile(NPC.GetSpawnSource_ForProjectile(), NPC.Center.X, NPC.Center.Y, 12, 0, 44, 20, 4, Main.myPlayer);
							Vector2 vector33 = new Vector2(NPC.position.X + (float)NPC.width * 0.5f, NPC.position.Y + (float)NPC.height * 0.5f);
							float num333 = player.position.X + (float)(player.width / 2) - vector33.X;
							float num334 = player.position.Y + (float)(player.height / 2) - vector33.Y;
				...
				else
				{
					if (NPC.ai[0] == 1f || NPC.ai[0] == 2f)
					{
						...
						if (NPC.ai[1] == 100f)
						{
							...
							else
							{
								...
								{
									Gore.NewGore(NPC.position, new Vector2((float)Main.rand.Next(-30, 31) * 0.2f, (float)Main.rand.Next(-30, 31) * 0.2f), ModContent.Find<ModGore>("Consolaria/OcramGore1").Type);
									Gore.NewGore(NPC.position, new Vector2((float)Main.rand.Next(-30, 31) * 0.2f, (float)Main.rand.Next(-30, 31) * 0.2f), ModContent.Find<ModGore>("Consolaria/OcramGore2").Type);
									Gore.NewGore(NPC.position, new Vector2((float)Main.rand.Next(-30, 31) * 0.2f, (float)Main.rand.Next(-30, 31) * 0.2f), ModContent.Find<ModGore>("Consolaria/OcramGore3").Type);
								}
					...
					else
					{
						...
						if (NPC.ai[1] == 0f)
						{
							...
							float num378 = player.position.X + (float)(player.width / 2) - (float)(num377 * 180) - vector38.X;
							float num379 = player.position.Y + (float)(player.height / 2) - 300f - vector38.Y;
								...
								if (Main.netMode != 1)
								{
									...
									if (Main.netMode != 1)
									{
										...
										if (NPC.localAI[1] > 8f)
										{
											...
											float num363 = player.position.X + (float)(player.width / 2) - (float)(num362 * 70) - vector38.X;
											float num364 = player.position.Y + (float)(player.height / 2) - vector38.Y;
											...
											num363 = player.position.X + (float)(player.width / 2) - zalupa - vector38.X;
											num364 = player.position.Y + (float)(player.height / 2) - zalupa - vector38.Y;
											float num366 = 10f;
											...
											Vector2 vector8 = new Vector2(NPC.position.X + (NPC.width / 2), NPC.position.Y + (NPC.height / 2));
											SoundEngine.PlaySound(2, (int)NPC.position.X, (int)NPC.position.Y, 33);
											int damage = Main.expertMode ? 54 : 20;
											float rotation = (float)Math.Atan2(vector8.Y - (player.position.Y + (player.height * 0.5f)), vector8.X - (player.position.X + (player.width * 0.5f)));								
											Projectile.NewProjectile(NPC.GetSpawnSource_ForProjectile(), vector8.X, vector8.Y, num363, num364, 83, damage, 2f, Main.myPlayer);
										}
									}
								}
							}
							...
							if (NPC.ai[2] >= 300f && NPC.ai[2] <= 500f)
							{
								...
								NPC.rotation = (float)Math.Atan2((double)NPC.velocity.Y, (double)NPC.velocity.X) - 1.57f;
								if (NPC.ai[2] % 30 == 0 && NPC.CountNPCS(ModContent.NPCType<ServantofOcram>()) <= 12)
								{
									SoundEngine.PlaySound(4, (int)NPC.position.X, (int)NPC.position.Y, 45, 0.8f, 0f);
									NPC.NewNPC(NPC.GetSpawnSourceForNPCFromNPCAI(), (int)NPC.Center.X - 40, (int)NPC.Center.Y, ModContent.NPCType<ServantofOcram>());
								}
							}
						...
						else
						{
							if (NPC.ai[1] == 1f)
							{
								...
								while (f <= 4f)
								{
									num54 = Projectile.NewProjectile(NPC.GetSpawnSource_ForProjectile(), vector8.X, vector8.Y, (float)((Math.Cos(rotation + f) * num48) * -1), (float)((Math.Sin(rotation + f) * num48) * -1), type, damage, 0f, Main.myPlayer);
								...
								float num385 = player.position.X + (float)(player.width / 2) - vector39.X;
								float num386 = player.position.Y + (float)(player.height / 2) - vector39.Y;
			...
			if (Main.expertMode && Phase2 && Main.rand.Next(600) == 0) {
				...
				if (Collision.CanHit(Vector3, 1, 1, player.position, player.width, player.height)) {
					if (Main.netMode != NetmodeID.MultiplayerClient) {
						if (player.active) {
							for (int i = 0; i < MissileProjectiles; i++) {
								SoundEngine.PlaySound(4, (int)NPC.position.X, (int)NPC.position.Y, 45, 0.8f, 0f);
								int Speed = 10;
								float SpawnX = Main.rand.Next(1000) - 500 + player.position.X;
								float SpawnY = -1400 - player.position.Y;
								...
								int Num33 = Projectile.NewProjectile(NPC.GetSpawnSource_ForProjectile(), Spawn.X, Spawn.Y, Velocity.X, Velocity.Y, ModContent.ProjectileType<OcramSkull>(), 25, 1f, Main.myPlayer, 0f, 0f);
								Main.projectile[Num33].velocity.X = Main.rand.Next(-200, 201) * 0.15f;
								Main.npc[Num33].velocity.Y = Main.rand.Next(-200, 201) * 0.15f;
		...
		public override void HitEffect(int hitDirection, double damage) {
			...
			if (NPC.life <= 0) {
				...
				Gore.NewGore(NPC.position, NPC.velocity, ModContent.Find<ModGore>("Gores/OcramGore1").Type, 1f);
				Gore.NewGore(NPC.position, NPC.velocity, ModContent.Find<ModGore>("Gores/OcramGore1").Type, 1f);
				Gore.NewGore(NPC.position, NPC.velocity, ModContent.Find<ModGore>("Gores/OcramGore1").Type, 1f);
				Gore.NewGore(NPC.position, NPC.velocity, ModContent.Find<ModGore>("Gores/OcramGore2").Type, 1.2f);
				Gore.NewGore(NPC.position, NPC.velocity, ModContent.Find<ModGore>("Gores/OcramGore2").Type, 1.2f);
				Gore.NewGore(NPC.position, NPC.velocity, ModContent.Find<ModGore>("Gores/OcramGore3").Type, 1f);
				Gore.NewGore(NPC.position, NPC.velocity, ModContent.Find<ModGore>("Gores/OcramGore3").Type, 1f);
				Gore.NewGore(NPC.position, NPC.velocity, ModContent.Find<ModGore>("Gores/OcramGore1").Type, 1f);
			}
		}
