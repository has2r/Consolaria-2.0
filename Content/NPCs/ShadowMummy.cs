...
namespace Consolaria.Content.NPCs {
	public class ShadowMummy : ModNPC {
		...
		public override void SetDefaults () {
			...
			Banner = NPC.type;
			BannerItem = ModContent.ItemType<Items.Placeable.Banners.ShadowMummyBanner>();
		}
		...
		public override void HitEffect (NPC.HitInfo hit) {
			...
			Dust.NewDust(NPC.position, NPC.width, NPC.height, DustID.Corruption, 2.5f * hit.HitDirection, -2.5f, 0, default, 1f);
			if (NPC.life <= 0) {
		...
		public override float SpawnChance (NPCSpawnInfo spawnInfo)
			=> (spawnInfo.Player.ZoneCorrupt && spawnInfo.Player.ZoneDesert && spawnInfo.SpawnTileY < Main.rockLayer && Main.hardMode) ?
			 SpawnCondition.DarkMummy.Chance * 0.1f : 0f;
	}
}