...
namespace Consolaria.Content.Projectiles.Enemies
{
	public class OcramScythe : ModProjectile
	{
		...
		public override void AI()
		{
			...
			/*if (Projectile.timeLeft == 299)
			{
				...
				while (vel < dustCount)
				{
					...
					int dust = Dust.NewDust(Projectile.Center, 0, 0, 27, 0f, 0f, 0, default(Color), 1f);
			...
			int dust2 = Dust.NewDust(Projectile.position, Projectile.width, Projectile.height, 27, Projectile.velocity.X * 0.2f, Projectile.velocity.Y * 0.2f, 100, default, 1.2f);
		...
		public override void Kill(int timeLeft)
		{
			...
			for (int i = 0; i < 30; i++)
			{
				int num506 = Dust.NewDust(new Vector2(Projectile.position.X, Projectile.position.Y), Projectile.width, Projectile.height, 27, Projectile.velocity.X, Projectile.velocity.Y, 100, default, 1.5f);
				Main.dust[num506].noGravity = true;
				Dust.NewDust(new Vector2(Projectile.position.X, Projectile.position.Y), Projectile.width, Projectile.height, 27, Projectile.velocity.X, Projectile.velocity.Y, 100, default, 1.1f);
			}
		}
