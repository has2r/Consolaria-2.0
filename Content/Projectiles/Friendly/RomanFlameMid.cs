    ...
    public class RomanFlameMid : ModProjectile
    {
        ...
        public override void Kill(int timeLeft) {
            ...
            if (Projectile.owner == Main.myPlayer) {
                ...
                for (int i = 0; i < 20; i++) {
                    int _dust = Dust.NewDust(Projectile.Center - position / 2f, (int)position.X, (int)position.Y, dustType, 0f, 0f, 0, default, 1f);
                    Main.dust[_dust].noGravity = true;
                    Main.dust[_dust].velocity *= 2.5f;
                    _dust = Dust.NewDust(Projectile.Center - position / 2f, (int)position.X, (int)position.Y, dustType, 0f, 0f, 0, default, 1.2f);
